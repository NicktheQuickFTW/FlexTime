...
// ⚠️  AI AGENTS: USE THESE TO WORK WITH THE NEW UNIFIED CONSTRAINT SYSTEM
// ========================================

/**
 * Get school ID from school name or abbreviation
 * @param {string} identifier - School name, short display, or abbreviation
 * @returns {number|null} School ID or null if not found
 */
export function getSchoolId(identifier) {
  const searchTerm = identifier.toLowerCase().replace(/[-\s]/g, '');
  
  // Search full members
  for (const [id, school] of Object.entries(BIG12_SCHOOLS)) {
    const names = [
      school.school.toLowerCase(),
      school.short_display.toLowerCase(),
      school.school_abbreviation.toLowerCase(),
      school.preferred_school_name.toLowerCase()
    ].map(n => n.replace(/[-\s]/g, ''));
  ...
  // Search affiliates
  for (const [id, school] of Object.entries(BIG12_AFFILIATES)) {
    const names = [
      school.school.toLowerCase(),
      school.short_display.toLowerCase(),
      school.school_abbreviation.toLowerCase()
...
/**
 * Get sport ID from sport name or code
 * @param {string} identifier - Sport name or code
 * @returns {number|null} Sport ID or null if not found
 */
export function getSportId(identifier) {
  const searchTerm = identifier.toLowerCase().replace(/['\s]/g, '');
  
  for (const [id, sport] of Object.entries(SPORTS)) {
    const names = [
      sport.name.toLowerCase().replace(/['\s]/g, ''),
...
/**
 * Get all team IDs for a specific sport
 * @param {number|string} sportIdentifier - Sport ID or name/code
 * @returns {Array<number>} Array of team IDs
 */
export function getTeamIdsForSport(sportIdentifier) {
  const sportId = typeof sportIdentifier === 'number' 
    ? sportIdentifier 
    : getSportId(sportIdentifier);
    
  if (!sportId) return [];
  
  const teams = generateBig12Teams();
  return teams
    .filter(team => team.sport_id === sportId)
    .map(team => team.team_id);
}

/**
 * Check if a school participates in a sport
 * @param {number|string} schoolIdentifier - School ID or name
 * @param {number|string} sportIdentifier - Sport ID or name
 * @returns {boolean} True if school participates in the sport
 */
export function schoolParticipatesInSport(schoolIdentifier, sportIdentifier) {
  const schoolId = typeof schoolIdentifier === 'number'
    ? schoolIdentifier
    : getSchoolId(schoolIdentifier);
    
  const sportId = typeof sportIdentifier === 'number'
    ? sportIdentifier
    : getSportId(sportIdentifier);
    
  if (!schoolId || !sportId) return false;
  
  const teamId = generateTeamId(schoolId, sportId);
  const teams = generateBig12Teams();
...
/**
 * Get venue ID for a school and sport
 * @param {number|string} schoolIdentifier - School ID or name
 * @param {number|string} sportIdentifier - Sport ID or name
 * @returns {number|null} Venue ID or null if not found
 */
export function getVenueForSport(schoolIdentifier, sportIdentifier) {
  const schoolId = typeof schoolIdentifier === 'number'
    ? schoolIdentifier
    : getSchoolId(schoolIdentifier);
  ...
  // Find venue type for this sport
  for (const [venueTypeId, venueType] of Object.entries(VENUE_TYPES)) {
    if (venueType.sport_codes.includes(sport.code)) {
      return generateVenueId(schoolId, parseInt(venueTypeId));
    }
  }
...
export default {
  ...
  generateBig12Teams,
  generateBig12Venues,
  // New constraint system helpers
  getSchoolId,
  getSportId,
  getTeamIdsForSport,
  schoolParticipatesInSport,
  getVenueForSport
};