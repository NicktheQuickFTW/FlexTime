...
/**
 * Enhanced Big 12 Men's Tennis Analysis - Summer 2025 Transfer Portal & Recruiting Focus
 * Comprehensive research execution for all Big 12 men's tennis programs
 */
...
class EnhancedMensTennisAnalysis {
  constructor() {
  ...
  async executeEnhancedMensTennisAnalysis() {
    console.log(`\n🎾 ENHANCED BIG 12 MEN'S TENNIS ANALYSIS - SUMMER 2025 FOCUS`);
  ...
  async executeEnhancedProjections(results) {
    console.log(`🎯 PHASE 1: Enhanced Men's Tennis Projections (Summer 2025 Focus)`);
    console.log(`📡 Executing parallel Perplexity research queries...\n`);

    const projectionPromises = this.teams.map(async (team, index) => {
      try {
        console.log(`   📊 [${index + 1}/${this.teams.length}] ${team} men's enhanced projections...`);
  ...
  async executeEnhancedCompassRatings(results) {
    ...
    for (let i = 0; i < this.teams.length; i++) {
      ...
      try {
        console.log(`   🧭 [${i + 1}/${this.teams.length}] ${team} men's enhanced COMPASS analysis...`);
  ...
  async saveResults(results) {
    ...
    try {
      ...
      // Save comprehensive men's tennis data
      const filePath = path.join(dataDir, 'summer_2025_mens_tennis_data.json');
      await fs.writeFile(filePath, JSON.stringify(results, null, 2));
      
      // Save execution summary
      const summaryPath = path.join(dataDir, 'summer_2025_mens_tennis_analysis.json');
      const summary = {
  ...
  generateExecutionSummary(results) {
    ...
    console.log(`\n🎯 ================================`);
    console.log(`🎾 ENHANCED MEN'S TENNIS ANALYSIS COMPLETE`);
    console.log(`🎯 ================================`);
    ...
    console.log(`✨ Enhanced Methods: Men's tennis-specific projections and COMPASS ratings`);
    console.log(`📈 Success Rate: ${Math.round((totalAnalyses / (this.teams.length * 2)) * 100)}%`);
    console.log(`\n🏆 Analysis includes:`);
    console.log(`   • Enhanced 2025-26 men's tennis projections with transfer portal focus`);
    console.log(`   • Current recruiting class status and international pipeline`);
    console.log(`   • Enhanced COMPASS ratings with men's tennis-specific methodology`);
    console.log(`   • Singles/doubles lineup construction and championship contention`);
    console.log(`\n🎯 Ready for enhanced men's tennis research insights! 🎾`);
  }
}

// Execute enhanced men's tennis analysis
async function main() {
  try {
    const analyzer = new EnhancedMensTennisAnalysis();
    await analyzer.executeEnhancedMensTennisAnalysis();
    process.exit(0);
  } catch (error) {
    console.error('❌ Enhanced men\'s tennis analysis failed:', error);
    process.exit(1);
  }
}
...
module.exports = EnhancedMensTennisAnalysis;