...
// Read the CSV file
const csvContent = fs.readFileSync('Sport Sponsorship 6.3.csv', 'utf8');
...
for (let i = 1; i < lines.length; i++) {
  ...
  const sportName = fields[0]?.replace(/﻿/g, ''); // Remove BOM
  const sponsorshipCount = parseInt(fields[1]);
  const legacyMembers = fields[2];
  
  // Check if this is a FlexTime-managed sport
  const sportId = sportMapping[sportName];
  if (!sportId) continue;
  
  // Parse legacy members (Big 12 schools only)
  if (legacyMembers) {
    const schools = legacyMembers.split(',').map(s => s.trim());
    
    schools.forEach(schoolName => {
      const schoolId = schoolMapping[schoolName];
      if (schoolId && !sportParticipation[sportId].includes(schoolId)) {
...
// Output the result
console.log('Big 12 Sport Participation Mapping (Updated from Sport Sponsorship 6.3.csv):');
console.log('==========================================================================');
console.log();
...
Object.entries(sportParticipation).forEach(([sportId, schools]) => {
  ...
  totalTeams += schools.length;
  console.log();
});

console.log(`Total FlexTime-managed teams: ${totalTeams}`);
console.log();

// Output JavaScript object format for neon-database.js
console.log('JavaScript Object for neon-database.js:');
console.log('======================================');
