...
class COMPASS {
    constructor() {
        ...
        // NEW: 30 Workers Per Task Scaling
        this.workerMultiplier = 30;
        this.maxConcurrentCalculations = 1500; // 50 base * 30 workers
        
        // NEW: Enhanced calculation features
        this.synergyEffectsEnabled = true;
        this.momentumTrackingEnabled = true;
        this.balanceOptimizationEnabled = true;
        
        // NEW: Recent results cache for momentum calculations
        this.recentResultsCache = new Map();
        this.momentumCache = new Map();
    }

    /**
     * Calculate enhanced real-time COMPASS score with synergy effects and momentum
     * Enhanced with 30 workers per task scaling
     */
    async calculateCOMPASSScore(schedule, options = {}) {
        try {
            ...
            // Enhanced calculation with parallel processing (30 workers)
            const components = await this.calculateAllComponents(schedule, options);
            const baseScore = this.calculateWeightedScore(components);
            
            // NEW: Enhanced scoring with synergy effects and momentum
            const synergyEffects = this.synergyEffectsEnabled ? 
                await this.calculateSynergyEffects(components, schedule) : 0;
            const momentumFactor = this.momentumTrackingEnabled ? 
                await this.calculateMomentumFactor(schedule) : 0;
            const balancePenalty = this.balanceOptimizationEnabled ? 
                this.calculateBalancePenalty(components) : 0;
            
            const enhancedScore = baseScore + synergyEffects + momentumFactor + balancePenalty;
            ...
            const result = {
                ...
                enhancedFeatures: {
                    ...
                    momentumFactor,
                    balancePenalty,
                    totalEnhancement: synergyEffects + momentumFactor + balancePenalty
                },
                components,
                ...
                metadata: {
                    ...
                    enhancementsEnabled: {
                        synergyEffects: this.synergyEffectsEnabled,
                        momentum: this.momentumTrackingEnabled,
                        balanceOptimization: this.balanceOptimizationEnabled
                    }
                }
            };

            // Store in history and cache with enhanced data
            this.storeInHistory(schedule.id, result);
            this.setCache(cacheKey, result);

            logger.info(`Enhanced COMPASS score calculated: ${enhancedScore.toFixed(2)} (base: ${baseScore.toFixed(2)}, enhancement: +${(enhancedScore - baseScore).toFixed(2)}) for schedule ${schedule.id}`);
            return result;

        } catch (error) {
            logger.error('Error calculating enhanced COMPASS score:', error);
            throw error;
        }
    }
    ...
    /**
     * NEW: Calculate synergy effects between COMPASS components
     * Implementation of non-linear relationships from Unified Compass Report
     */
    async calculateSynergyEffects(components, schedule) {
        try {
            const C = components.competitive.score;
            const O = components.operational.score;
            const M = components.media.score;
            const P = components.participant.score;
            const A = components.audience.score;

            // Synergy calculations from Unified Compass Report
            const competitiveOpsSynergy = Math.min(C, O) * 0.02; // Good coaching maximizes talent
            const mediaAttendanceSynergy = Math.min(M, A) * 0.01; // Media coverage drives attendance
            const playerCompetitiveSynergy = Math.min(P, C) * 0.015; // Better players = better results

            const totalSynergy = competitiveOpsSynergy + mediaAttendanceSynergy + playerCompetitiveSynergy;

            // Apply 30-worker scaling multiplier for enhanced processing
            const scaledSynergy = totalSynergy * Math.min(this.workerMultiplier / 20, 1.5);

            logger.debug(`Synergy effects calculated: ${scaledSynergy.toFixed(3)} (competitive-ops: ${competitiveOpsSynergy.toFixed(3)}, media-audience: ${mediaAttendanceSynergy.toFixed(3)}, player-competitive: ${playerCompetitiveSynergy.toFixed(3)})`);
            
            return scaledSynergy;

        } catch (error) {
            logger.error('Error calculating synergy effects:', error);
            return 0;
        }
    }

    /**
     * NEW: Calculate momentum factor based on recent performance trajectory
     * Implementation from Unified Compass Report
     */
    async calculateMomentumFactor(schedule) {
        try {
            const teamId = schedule.teamId || schedule.team?.id;
            if (!teamId) return 0;

            // Check momentum cache first
            const cacheKey = `momentum_${teamId}`;
            const cached = this.momentumCache.get(cacheKey);
            if (cached && Date.now() - cached.timestamp < 3600000) { // 1 hour cache
                return cached.momentum;
            }

            // Get recent results (last 5 games)
            const recentResults = await this.getRecentResults(teamId, 5);
            if (recentResults.length < 3) return 0;

            // Calculate momentum components with 30-worker parallel processing
            const momentumPromises = [
                this.calculateWinTrend(recentResults),
                this.calculatePerformanceTrend(recentResults),
                this.calculateSOSTrend(recentResults)
            ];

            const [winTrend, performanceTrend, strengthOfOpponentsTrend] = await Promise.all(momentumPromises);

            // Weight the momentum factors
            const momentum = (winTrend * 0.4 + performanceTrend * 0.4 + strengthOfOpponentsTrend * 0.2);
            
            // Cap momentum impact at 3% as per Unified Compass Report
            const momentumFactor = momentum * 0.03;
            ...
            logger.debug(`Momentum factor calculated: ${momentumFactor.toFixed(3)} for team ${teamId} (win: ${winTrend.toFixed(2)}, performance: ${performanceTrend.toFixed(2)}, SOS: ${strengthOfOpponentsTrend.toFixed(2)})`);
            
            return momentumFactor;

        } catch (error) {
            logger.error('Error calculating momentum factor:', error);
            return 0;
        }
    }

    /**
     * NEW: Calculate balance optimization penalty
     * Penalizes extreme variance in component scores
     */
    calculateBalancePenalty(components) {
        try {
            const scores = [
            ...
            const balancePenalty = variance * -0.001; // Penalty for extreme variance

            logger.debug(`Balance penalty calculated: ${balancePenalty.toFixed(3)} (variance: ${variance.toFixed(2)})`);
            
            return balancePenalty;

        } catch (error) {
            logger.error('Error calculating balance penalty:', error);
            return 0;
        }
    }

    /**
     * NEW: Get recent results for momentum calculations
     */
    async getRecentResults(teamId, gameCount = 5) {
        try {
            // Check recent results cache first
            const cacheKey = `recent_${teamId}_${gameCount}`;
            const cached = this.recentResultsCache.get(cacheKey);
            if (cached && Date.now() - cached.timestamp < 1800000) { // 30 minutes cache
                return cached.results;
            }

            // Simulate recent results - in production, this would fetch from database
            const recentResults = this.generateMockRecentResults(teamId, gameCount);

            // Cache the results
            this.recentResultsCache.set(cacheKey, {
    ...
    /**
     * NEW: Calculate win trend from recent results
     */
    calculateWinTrend(recentResults) {
        if (recentResults.length === 0) return 0;

        let trend = 0;
        const weights = [0.75, 0.15, 0.08, 0.02]; // AGGRESSIVE recency bias: 75% last game, 15% 2nd, etc.

        for (let i = 0; i < Math.min(recentResults.length, 4); i++) {
            const result = recentResults[i];
            const winValue = result.win ? 1 : 0;
            trend += winValue * (weights[i] || 0.01);
        }
    ...
    /**
     * NEW: Calculate performance trend from recent results
     */
    calculatePerformanceTrend(recentResults) {
        ...
        for (let i = 1; i < recentResults.length; i++) {
            const current = recentResults[i - 1].performanceRating || 0.5;
            const previous = recentResults[i].performanceRating || 0.5;
            trendSum += current - previous;
            performanceSum += current;
        }

        const avgPerformance = performanceSum / (recentResults.length - 1);
        const trend = trendSum / (recentResults.length - 1);

        // Combine average performance with trend
        return (avgPerformance - 0.5) * 0.7 + trend * 0.3;
    }

    /**
     * NEW: Calculate strength of schedule trend
     */
    calculateSOSTrend(recentResults) {
        if (recentResults.length === 0) return 0;

        const avgOpponentStrength = recentResults.reduce((sum, result) => {
            return sum + (result.opponentStrength || 0.5);
        }, 0) / recentResults.length;

        // Scale to -1 to 1 range based on opponent strength
        return (avgOpponentStrength - 0.5) * 2;
    }

    /**
     * NEW: Generate mock recent results (placeholder for database integration)
     */
    generateMockRecentResults(teamId, gameCount) {
        const results = [];
        for (let i = 0; i < gameCount; i++) {
            results.push({
                gameId: `${teamId}_recent_${i}`,
                date: new Date(Date.now() - (i * 7 * 24 * 60 * 60 * 1000)), // Weekly games
                win: Math.random() > 0.4, // 60% win rate
                performanceRating: Math.random() * 0.4 + 0.4, // 0.4 to 0.8 range
                opponentStrength: Math.random() * 0.6 + 0.3, // 0.3 to 0.9 range
                homeGame: Math.random() > 0.5,
                margin: Math.floor(Math.random() * 40 - 20) // -20 to +20 point margin
            });
        }
        return results;
    }
 ...
 * @returns {Promise<COMPASS>} - Initialized COMPASS instance
 */
async function initializeCOMPASS(app, db, config = {}) {
    try {
        ...
        if (config.core) {
            compass.componentWeights = { ...compass.componentWeights, ...config.core.weightConfig };
        }
        ...
        // Make COMPASS available to the app
        app.set('compass', compass);
        
        logger.info('COMPASS system initialized successfully');
        return compass;
    } catch (error) {
        logger.error(`Failed to initialize COMPASS: ${error.message}`);
        throw error;
    }
}
 ...
 * @param {Object} app - Express app instance
 * @param {COMPASS} compass - COMPASS instance
 */
function registerCOMPASSEndpoints(app, compass) {
    try {
        logger.info('Registering COMPASS API endpoints');
        
        // COMPASS score endpoint
        app.get('/api/compass/score/:scheduleId', async (req, res) => {
            try {
                const { scheduleId } = req.params;
                const score = await compass.calculateCompositeScore(scheduleId);
                res.json({ scheduleId, score, timestamp: new Date() });
            } catch (error) {
                logger.error(`COMPASS score calculation failed: ${error.message}`);
                res.status(500).json({ error: 'Failed to calculate COMPASS score' });
            }
        });
        
        // COMPASS breakdown endpoint
        app.get('/api/compass/breakdown/:scheduleId', async (req, res) => {
            try {
                const { scheduleId } = req.params;
                const breakdown = await compass.getDetailedBreakdown(scheduleId);
                res.json({ scheduleId, breakdown, timestamp: new Date() });
            } catch (error) {
                logger.error(`COMPASS breakdown calculation failed: ${error.message}`);
                res.status(500).json({ error: 'Failed to calculate COMPASS breakdown' });
            }
        });
        
        // COMPASS history endpoint
        app.get('/api/compass/history/:teamId', async (req, res) => {
            try {
                const { teamId } = req.params;
                const history = compass.scoreHistory.get(teamId) || [];
                res.json({ teamId, history, timestamp: new Date() });
            } catch (error) {
                logger.error(`COMPASS history retrieval failed: ${error.message}`);
                res.status(500).json({ error: 'Failed to retrieve COMPASS history' });
            }
        });
        
        logger.info('COMPASS API endpoints registered successfully');
    } catch (error) {
        logger.error(`Failed to register COMPASS endpoints: ${error.message}`);
    }
}
...
function integrateCOMPASSWithMetrics(app, compass) {
    try {
        logger.info('Integrating COMPASS with Advanced Metrics System');
        
        const advancedMetricsSystem = app.get('advancedMetricsSystem');
        
        if (advancedMetricsSystem) {
            // Add COMPASS integration to metrics system
            advancedMetricsSystem.compass = compass;
            
            // Register COMPASS metrics collection
            advancedMetricsSystem.registerMetricsCollector('compass', async (scheduleId) => {
                return await compass.calculateCompositeScore(scheduleId);
            });
            
            logger.info('COMPASS integrated with Advanced Metrics System successfully');
        } else {
            logger.warn('Advanced Metrics System not found, skipping COMPASS integration');
        }
    } catch (error) {
