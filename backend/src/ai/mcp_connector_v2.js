/**
 * Enhanced MCP Connector for the FlexTime multi-agent system.
 * 
 * This module implements the integration with multiple Model Context Protocol servers
 ...
 */
class MCPConnectorV2 {
  /**
   * Initialize a new Enhanced MCP Connector.
   * 
  ...
  constructor(config = {}) {
    ...
    logger.info('Enhanced MCP Connector initialized');
    logger.info(`MCP integration enabled: ${this.enabled}`);
    logger.info(`Default MCP server: ${this.defaultServer}`);
  }
  
  /**
   * Initialize HTTP clients for all enabled MCP servers.
   * 
   * @private
   */
  initializeClients() {
  ...
  /**
   * Initialize an HTTP client for a specific MCP server.
   * 
  ...
  initializeClient(serverName, serverConfig) {
    try {
      ...
      logger.info(`Initialized client for MCP server: ${serverName}`);
    } catch (error) {
      logger.error(`Failed to initialize client for MCP server ${serverName}: ${error.message}`);
    }
  }
  
  /**
   * Get the appropriate MCP server for a specific agent or task.
   * 
  ...
  async sendRequest(options = {}) {
    ...
    try {
      logger.info(`Sending request to ${serverName} MCP server (model: ${model})`);
  ...
  /**
   * Format and send a request to the specified MCP server.
   * 
   * @param {string} serverName - Name of the MCP server
  ...
  async checkAvailability() {
    ...
    for (const [serverName, client] of Object.entries(this.clients)) {
      try {
        logger.info(`Checking availability of MCP server: ${serverName}`);
        ...
        if (response.status === 200) {
          logger.info(`MCP server ${serverName} is available`);
          return true;
        }
      } catch (error) {
        logger.error(`MCP server ${serverName} availability check failed: ${error.message}`);
      }
    }
  ...
  /**
   * Get available AI models from all enabled MCP servers.
   * 
  ...
  configure(config = {}) {
    ...
    logger.info('Enhanced MCP Connector reconfigured');
    logger.info(`MCP integration enabled: ${this.enabled}`);
    logger.info(`Default MCP server: ${this.defaultServer}`);
  }
}

module.exports = MCPConnectorV2;
