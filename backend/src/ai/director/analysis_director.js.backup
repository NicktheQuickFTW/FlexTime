/**
 * Analysis Director Agent for the FlexTime multi-agent system.
 * 
 * This module implements the Analysis Director Agent that coordinates
 * all analysis-related specialized agents.
 */
 ...
 */
class AnalysisDirectorAgent extends Agent {
  /**
  ...
  constructor(config, mcpConnector) {
    super('analysis_director', 'director', mcpConnector);
    this.config = config;
    this.specializedAgents = {};
    
    logger.info('Analysis Director Agent initialized');
  }
  ...
  initializeAgents(communicationManager) {
    ...
    const ScheduleAnalysisAgent = require('../specialized/schedule_analysis_agent');
    const VisualizationAgent = require('../specialized/visualization_agent');
    const ReportingAgent = require('../specialized/reporting_agent');
    
    // Create specialized agents
    this.specializedAgents.analysis = new ScheduleAnalysisAgent(this.mcpConnector);
    this.specializedAgents.visualization = new VisualizationAgent(this.mcpConnector);
    this.specializedAgents.reporting = new ReportingAgent(this.mcpConnector);
    ...
    for (const [key, agent] of Object.entries(this.specializedAgents)) {
      ...
      agent.agentId = `analysis.${key}`;
      agent.register(communicationManager);
      agent.start();
    }
    
    logger.info('All analysis specialized agents initialized and started');
  }
  ...
  async _processTask(task) {
    logger.info(`Analysis Director processing task: ${task.taskType}`);
  ...
  _determineResponsibleAgent(task) {
    ...
    const taskTypeMapping = {
      'analyze_schedule': 'analysis.analysis',
      'visualize_data': 'analysis.visualization',
      'generate_report': 'analysis.reporting'
    };
    ...
    if (taskDescription.includes('analyze') || 
        taskDescription.includes('evaluate') || 
        taskDescription.includes('assess')) {
      return 'analysis.analysis';
    }
    
    if (taskDescription.includes('visualize') || 
        taskDescription.includes('chart') || 
        taskDescription.includes('graph')) {
      return 'analysis.visualization';
    }
    
    if (taskDescription.includes('report') || 
        taskDescription.includes('document') || 
        taskDescription.includes('summarize')) {
      return 'analysis.reporting';
    }
    
    // Default to analysis if no match found
    return 'analysis.analysis';
  }
  ...
  _processSubtaskResult(taskId, subtaskId, result) {
    logger.info(`Analysis Director received result for subtask ${subtaskId}`);
  ...
  _processSubtaskFailure(taskId, subtaskId, error) {
    logger.error(`Analysis Director received failure for subtask ${subtaskId}: ${error}`);
...
module.exports = { AnalysisDirectorAgent };
