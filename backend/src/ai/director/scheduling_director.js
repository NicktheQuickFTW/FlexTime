/**
 * Scheduling Director Agent for the FlexTime multi-agent system.
 * 
 * This module implements the Scheduling Director Agent that coordinates
 * all scheduling-related specialized agents.
 */
...
/**
 * Scheduling Director Agent that coordinates scheduling-related specialized agents.
 */
class SchedulingDirectorAgent extends Agent {
  /**
  ...
  constructor(config, mcpConnector) {
    super('scheduling_director', 'director', mcpConnector);
    this.config = config;
    this.specializedAgents = {};
    
    logger.info('Scheduling Director Agent initialized');
  }
  ...
  initializeAgents(communicationManager) {
    ...
    const AlgorithmSelectionAgent = require('../specialized/algorithm_selection_agent');
    const ConstraintManagementAgent = require('../specialized/constraint_management_agent');
    const ScheduleOptimizationAgent = require('../specialized/schedule_optimization_agent');
    
    // Create specialized agents
    this.specializedAgents.algorithm = new AlgorithmSelectionAgent(this.mcpConnector);
    this.specializedAgents.constraint = new ConstraintManagementAgent(this.mcpConnector);
    this.specializedAgents.optimization = new ScheduleOptimizationAgent(this.mcpConnector);
    ...
    for (const [key, agent] of Object.entries(this.specializedAgents)) {
      ...
      agent.agentId = `scheduling.${key}`;
      agent.register(communicationManager);
      agent.start();
    }
    
    logger.info('All scheduling specialized agents initialized and started');
  }
  ...
  async _processTask(task) {
    logger.info(`Scheduling Director processing task: ${task.taskType}`);
  ...
  _determineResponsibleAgent(task) {
    ...
    const taskTypeMapping = {
      'select_algorithm': 'scheduling.algorithm',
      'analyze_constraints': 'scheduling.constraint',
      'optimize_schedule': 'scheduling.optimization',
      'generate_schedule': 'scheduling.optimization'
    };
    ...
    if (taskDescription.includes('algorithm') || 
        taskDescription.includes('select') || 
        taskDescription.includes('choose')) {
      return 'scheduling.algorithm';
    }
    
    if (taskDescription.includes('constraint') || 
        taskDescription.includes('rule') || 
        taskDescription.includes('requirement')) {
      return 'scheduling.constraint';
    }
    
    if (taskDescription.includes('optimize') || 
        taskDescription.includes('improve') || 
        taskDescription.includes('generate')) {
      return 'scheduling.optimization';
    }
    
    // Default to optimization if no match found
    return 'scheduling.optimization';
  }
  ...
  _processSubtaskResult(taskId, subtaskId, result) {
    logger.info(`Scheduling Director received result for subtask ${subtaskId}`);
  ...
  _processSubtaskFailure(taskId, subtaskId, error) {
    logger.error(`Scheduling Director received failure for subtask ${subtaskId}: ${error}`);
...
module.exports = { SchedulingDirectorAgent };
