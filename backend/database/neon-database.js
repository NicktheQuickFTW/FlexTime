...
class NeonDatabase {
  ...
  async initializeTables() {
    ...
    try {
      ...
      // Teams table already exists with team_id column
      // Schedules table already exists with schedule_id column
      console.log('ℹ️  Using existing teams and schedules tables');
  ...
  async insertBig12Teams() {
    console.log('🏟️ Checking Big 12 Conference teams...');
    
    // Skip team insertion since teams already exist in database with different ID structure
    console.log('ℹ️  Teams already exist in database (504 teams found), skipping insertion');
    return;
    ...
    try {
      for (const team of big12Teams) {
        await client.query(`
          INSERT INTO teams (team_id, name, abbreviation, conference, location, colors, venue_info)
          ...
          ON CONFLICT (team_id) DO UPDATE SET
            ...
            updated_at = NOW()
        `, [
          parseInt(Math.random() * 100000), // Generate random integer ID to avoid conflicts
          team.name,
          team.abbreviation,
          'big12',
