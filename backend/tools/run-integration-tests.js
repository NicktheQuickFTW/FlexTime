...
async function runIntegrationTests() {
  try {
    ...
    if (options.test) {
      // Run a specific test
      const testPath = path.resolve(__dirname, '../tests/integration-tests', `${options.test}.test.js`);
      testPaths.push(testPath);
    } else {
      // Run all integration tests
      testPaths = [
        path.resolve(__dirname, '../tests/integration-tests/agent-system-integration.test.js'),
        path.resolve(__dirname, '../tests/integration-tests/intelligence-engine-integration.test.js'),
        path.resolve(__dirname, '../tests/integration-tests/mcp-big12-integration.test.js')
      ];
    }
