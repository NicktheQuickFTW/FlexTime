...
const generateSampleGames = () => {
  const games = []
  const teams = big12Teams.slice(0, 8) // Use first 8 teams for demo
  const today = new Date()
  
  // Generate games for next 3 months
  for (let i = 0; i < 30; i++) {
    const gameDate = new Date(today)
    gameDate.setDate(today.getDate() + i * 3)
    
    const homeTeam = teams[Math.floor(Math.random() * teams.length)]
    const awayTeam = teams.filter(t => t.id !== homeTeam.id)[Math.floor(Math.random() * (teams.length - 1))]
    
    games.push({
      day: gameDate,
      events: [{
        id: i,
        name: `${awayTeam.shortName} @ ${homeTeam.shortName}`,
...
const builderTools = [
  ...
  { icon: <GanttChart className="w-5 h-5" />, color: 'green', label: 'Gantt View' },
  { icon: <Grid3X3 className="w-5 h-5" />, color: 'purple', label: 'Matrix View' },
  { icon: <MapPin className="w-5 h-5" />, color: 'orange', label: 'Venues' },
  { icon: <Users className="w-5 h-5" />, color: 'red', label: 'Teams' },
  { icon: <Trophy className="w-5 h-5" />, color: 'indigo', label: 'Championships' }
]

interface FTBuilderWithCalendarProps {
  className?: string
}

export function FTBuilderWithCalendar({ className }: FTBuilderWithCalendarProps) {
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-400">
                Full-featured calendar with Gantt and matrix views for Big 12 scheduling
              </p>
            </div>
      ...
      {!isFullscreen && (
        ...
        >
          <div className="max-w-7xl mx-auto px-6 mb-4">
            <h3 className="text-lg font-semibold text-cyan-400 mb-2">View Modes</h3>
          </div>
          ...
          >
            ...
            <div className="flex items-center justify-between p-4 border-b border-white/10">
              <h1 className="text-lg font-semibold bg-gradient-to-r from-black to-[color:var(--ft-neon)] bg-clip-text text-transparent">
                FlexTime Schedule Calendar - {sports.find(s => s.id === selectedSport)?.name}
              </h1>
              <FlexTimeSecondaryButton 
                className="px-4 py-2"
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between text-sm">
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-white/60">Calendar Active</span>
              </div>
              ...
              <div className="text-white/60">
                Games: <span className="text-cyan-400 font-semibold">{gamesData.length}</span>
              </div>
            </div>
            <div className="text-xs bg-gradient-to-r from-black to-[color:var(--ft-neon)] bg-clip-text text-transparent opacity-70">
              FlexTime Builder Calendar • Full scheduling suite
            </div>
          </div>
        </motion.footer>
      )}
