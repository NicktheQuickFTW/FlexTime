...
function ScheduleBuilderPage() {
  ...
  const handleAIGeneration = async () => {
    setIsLoading(true);
    try {
      const newSchedule = await ScheduleBuilderAPI.generateSchedule('football', {
  ...
  const handleOptimization = async () => {
    setIsLoading(true);
    try {
      const optimizedSchedule = await ScheduleBuilderAPI.optimizeSchedule(schedule.id);
  ...
  const handleExportSchedule = async (format) => {
    try {
      await ScheduleBuilderAPI.exportSchedule(schedule.id, format);
    } catch (error) {
...
function addScheduleBuilderToNavigation() {
  ...
  if (navItems.length > 0) {
    ...
    scheduleBuilderLink.href = '/schedule-builder';
    scheduleBuilderLink.className = 'nav-link-modern';
    scheduleBuilderLink.innerHTML = '▲ SCHEDULE BUILDER';
    scheduleBuilderLink.onclick = (e) => {
      e.preventDefault();
      showScheduleBuilder();
    };
    
    // Insert after the first nav item
    navItems[0].parentNode.insertBefore(scheduleBuilderLink, navItems[1]);
  }
}
...
function showScheduleBuilder() {
  ...
  if (appContainer) {
    ...
    ReactDOM.render(
      React.createElement(ScheduleBuilderPage),
      appContainer
    );
  }
}
...
function initializeScheduleBuilder() {
  ...
  window.FlexTimeScheduleBuilder = {
    show: showScheduleBuilder,
    API: ScheduleBuilderAPI,
    components: {
      ScheduleBuilderApp,
      ScheduleBuilderPage,
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initializeScheduleBuilder);
} else {
  initializeScheduleBuilder();
}

// Export all components
window.ScheduleBuilderPage = ScheduleBuilderPage;
...
window.ScheduleBuilderAPI = ScheduleBuilderAPI;